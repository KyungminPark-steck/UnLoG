# coding=utf-8
# epoch X data 개수 = steps X effective_batch_size 
# clean.csv => 1 X ?? = 422 X 32 
import torch
...
def parse_config():
    ...
    parser.add_argument("--use_mlm_loss", type=str, default='True', 
        help='Whether use MLM loss during training or not.')
    ...
    #parser.add_argument("--mlm_coeff", type=float, default=1)
...
if __name__ == '__main__':
    ...
    #print("MLE coeff:", args.mlm_coeff)
    print ('Initializing model...')
    print ("CL;", args.use_cl_loss)
    print("MLE:", args.use_mlm_loss)
    ...
    if args.from_scratch == 'True':
        ...
        from_scratch = False
    #print(args.use_mlm_loss, args.from_scratch)
    if args.use_mlm_loss == 'True':
        use_mlm_loss = True
    else:
        use_mlm_loss = False
        ...
        for batch_data in data:
            ...
            if not use_mlm_loss:
                ...
                if input_ids.is_cuda:
                    mle_loss = mle_loss.cuda(input_ids.get_device())
            ...
            # cl_loss 계산 여부에 따라 train_cl_loss에 추가
            if use_cl_loss:
                train_cl_loss += cl_loss.mean().item()
            else:
                train_cl_loss += 0.
